...
const Search = ({ isMobile = false, onClose }) => {
  ...
  const highlightMatch = (name = "", search = "") => {
    ...
    return name.split(regex).map((part, index) =>
      part.toLowerCase() === search.toLowerCase() ? (
        <span
          key={index}
          className="bg-indigo-100 text-indigo-800 rounded-sm px-0.5"
        >
          {part}
        </span>
      ) : (
        ...
        >
          ...
          <div className="ml-3">
            <h5 className="font-medium text-gray-900 flex flex-wrap items-center">
              ...
              <span className="ml-2 px-1.5 py-0.5 text-xs rounded-md bg-gray-100 text-gray-500">
                @{user?.username || "user"}
              </span>
            </h5>
  ...
  return (
    <div
      className="hidden md:flex flex-grow items-center mx-4 relative"
      ref={searchMenuRef}
    >
